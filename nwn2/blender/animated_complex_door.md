...
# Tutorial: Animating a complex door

This tutorial uses Blender 3.3.1.

# What is a complex animated door?
...
# Creating an animated complex door
...
## Step 2: Export a MDB file with the base part

1. Export to MDB only the base part.
2. Do the same for each attached part.

## Step 2: Export a MDB file with the first attached part.

## Step 2: Export a MDB file with the second attached part.
...
## Step 4: Export the idle animation
...
## Step 4: Export the OPEN1 animation

## Step 4: Export the OPEN2 animation

## Step 5: Edit doortypes.2da
...
the row of the door in **doortypes.2da** as follows:
...
2. Column **NWN2_Skeleton**: the name of the GR2 file containing the skeleton
   without the file extension. In our example: **PLC_ML_CHESTM01**.
3. Column **AttachedModelName**: if the door has only one attached part,
   you can just set the name of the attached part. If the door has more
...
4. Column **SoundAppType**: this is a reference to **placeableobjsnds.2da**,
   which defines a "door sound set" consisting of entries for sound to
